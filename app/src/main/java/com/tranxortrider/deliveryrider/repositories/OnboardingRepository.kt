...
class OnboardingRepository {
    ...
    fun setupProfile(
        ...
        db.collection("users")
                ...
                db.collection("riders")
                    ...
                    .addOnFailureListener { e ->
                        ...
                        if (e.message?.contains("NOT_FOUND") == true) {
                            createNewRiderProfile(userId, fullName, phoneNumber, onResponse)
                        } else {
            ...
            .addOnFailureListener { e ->
                ...
                // If the document doesn't exist yet, create it
                if (e.message?.contains("NOT_FOUND") == true) {
                    createNewUserProfile(userId, fullName, phoneNumber, address, city, state, zipCode, vehicleType, vehicleMake, vehicleModel, vehiclePlate, onResponse)
                } else {
     ...
     */
    private fun createNewRiderProfile(
        userId: String,
        fullName: String,
        phoneNumber: String,
        onResponse: (Boolean, String, SetupProfileResponse?) -> Unit
                ...
                db.collection("users")
                    ...
                    .addOnSuccessListener { document ->
                        if (document.exists()) {
                            ...
                            onResponse(false, "User profile not found", null)
                        }
                    }
                    .addOnFailureListener { e ->
                        Log.e(TAG, "Error getting user profile", e)
                        onResponse(false, "Failed to get user profile: ${e.message}", null)
                    }
            }
            .addOnFailureListener { e ->
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    while (fileUrl == null && retryCount <= maxRetries) {
                        ...
                        fileUrl = uploadToSupabase(context, finalFile, storagePath)
                        retryCount++
                    }
     ...
     */
    private suspend fun uploadToSupabase(context: Context, file: File, path: String): String? {
        return try {
            ...
            if (!SupabaseClient.isNetworkAvailable(context)) {
                Log.e(TAG, "No internet connection available")
                return null
            }

            // Check if Supabase is reachable
            if (!SupabaseClient.isSupabaseReachable()) {
                Log.e(TAG, "Supabase server is not reachable")
                return null
            }
            ...
            // Upload with options and timeout
            ...
            when (e) {
                is UnknownHostException -> {
                    Log.e(TAG, "Network error: Unable to resolve Supabase hostname", e)
                }
                is java.net.SocketTimeoutException -> {
                    Log.e(TAG, "Network error: Connection to Supabase timed out", e)
                }
                is java.net.ConnectException -> {
                    Log.e(TAG, "Network error: Failed to connect to Supabase", e)
                }
                else -> {
    ...
    suspend fun getUserDocuments(userId: String): Result<List<Document>> {
        return try {
            val documentsSnapshot = db.collection("userDocuments")
                .whereEqualTo("userId", userId)
                .get()
                .await()
            
            val documents = documentsSnapshot.documents.map { doc ->
                Document(
                    id = doc.id,
                    userId = doc.getString("userId") ?: "",
                    type = doc.getString("documentType") ?: "",
                    name = doc.getString("fileName") ?: "",
                    fileUrl = doc.getString("fileUrl") ?: "",
                    status = doc.getString("status") ?: "pending",
                    uploadedAt = (doc.getTimestamp("uploadedAt")?.toDate()?.time ?: 0),
                    updatedAt = (doc.getTimestamp("updatedAt")?.toDate()?.time 
                        ?: doc.getTimestamp("uploadedAt")?.toDate()?.time ?: 0),
                    reviewedAt = (doc.getTimestamp("reviewedAt")?.toDate()?.time ?: 0),
                    reviewedBy = doc.getString("reviewedBy") ?: "",
                    rejectionReason = doc.getString("rejectionReason") ?: "",
                    storageProvider = doc.getString("storageProvider") ?: "firebase",
     ...
     */
    fun resubmitDocument(
        context: Context,
        userId: String,
        documentId: String,
        documentType: DocumentType,
        documentUri: Uri,
        onResponse: (Boolean, String, DocumentUploadResponse?) -> Unit
    ) {
        try {
            // First get the old document to find its storage path
            db.collection("userDocuments")
                .document(documentId)
                .get()
                .addOnSuccessListener { documentSnapshot ->
                    ...
                    // Delete the old document from storage if it's in Supabase
                    if (storageProvider == "supabase" && storagePath != null) {
                        GlobalScope.launch(Dispatchers.IO) {
                            try {
                                documentsBucket.delete(storagePath)
                            } catch (e: Exception) {
                                Log.e(TAG, "Error deleting old document from Supabase", e)
                                // Continue anyway, as we're replacing the document
                            }
                        }
                    }
                    ...
                    db.collection("userDocuments")
                        ...
                        .addOnSuccessListener {
                            ...
                            uploadDocument(context, userId, documentType, documentUri) { success, uploadMessage, fileUrl ->
                                val response = if (success && fileUrl != null) {
                                ...
                                onResponse(success, uploadMessage, response)
                            }
                        }
                        .addOnFailureListener { e ->
                            Log.e(TAG, "Error deleting old document", e)
                            onResponse(false, "Failed to resubmit document: ${e.message}", null)
                        }
                }
                .addOnFailureListener { e ->
                    Log.e(TAG, "Error getting document details", e)
                    onResponse(false, "Failed to get document details: ${e.message}", null)
                }
        } catch (e: Exception) {
            Log.e(TAG, "Error in resubmitDocument", e)
            onResponse(false, "Error in resubmitDocument: ${e.message}", null)
        }
    }
     ...
     */
    suspend fun checkAllDocumentsSubmitted(userId: String): Result<Boolean> {
        return try {
            ...
            val submittedTypes = documentsSnapshot.documents.mapNotNull { doc ->
                doc.getString("documentType")
            }
            
            val allSubmitted = requiredDocuments.all { required ->
                submittedTypes.contains(required.name)
            }
            
            Result.success(allSubmitted)
        } catch (e: Exception) {
            Log.e(TAG, "Error checking document submission status", e)
            Result.failure(e)
        }
    }
